...
static bool thread_heap_diag_enabled() {
    static const bool enabled =
        std::getenv("FARLIB_THREAD_HEAP_DIAG") != nullptr;
    return enabled;
}
typedef struct {
...
void generate(Transformer* transformer, Tokenizer* tokenizer, Sampler* sampler,
    ...
    while (pos < steps) {
        ...
        if (thread_heap_diag_enabled() &&
            (pos == 1 || (pos % 4) == 0 || pos == steps)) {
            allocator::print_thread_heap_diagnostics("llama_generate", 0,
                                                     pos);
        }
...
int main(int argc, char* argv[]) {
    ...
    if (thread_heap_diag_enabled()) {
        allocator::print_thread_heap_diagnostics("llama_init", 0, 0);
    }
    ...
    // run!
    const int64_t profile_read_posts_before =
        profile::collect_rdma_read_post_count();
    const int64_t profile_read_bytes_before =
        profile::collect_rdma_read_post_bytes();
    const int64_t profile_write_posts_before =
        profile::collect_rdma_write_post_count();
    const int64_t profile_write_bytes_before =
        profile::collect_rdma_write_post_bytes();
    const int64_t profile_dirty_evict_bytes_before =
        profile::collect_dirty_evict_bytes();
    auto start = get_cycles();
    profile::begin_allocation_wait_diagnostics(get_thread_count());
    profile::start_work();
    ...
    if (thread_heap_diag_enabled()) {
        allocator::print_thread_heap_diagnostics("llama_benchmark_end", 0,
                                                 steps);
    }
    auto end = get_cycles();
    std::cout << "profile rdma read posts: "
              << profile::collect_rdma_read_post_count() -
                     profile_read_posts_before
              << std::endl;
    std::cout << "profile rdma read bytes: "
              << profile::collect_rdma_read_post_bytes() -
                     profile_read_bytes_before
              << std::endl;
    std::cout << "profile rdma write posts: "
              << profile::collect_rdma_write_post_count() -
                     profile_write_posts_before
              << std::endl;
    std::cout << "profile rdma write bytes: "
              << profile::collect_rdma_write_post_bytes() -
                     profile_write_bytes_before
              << std::endl;
    const int64_t profile_dirty_evict_bytes =
        profile::collect_dirty_evict_bytes() -
        profile_dirty_evict_bytes_before;
    const int64_t profile_write_bytes =
        profile::collect_rdma_write_post_bytes() - profile_write_bytes_before;
    std::cout << "profile dirty eviction bytes: "
              << profile_dirty_evict_bytes << std::endl;
    std::cout << "profile clean eviction write bytes: "
              << profile_write_bytes - profile_dirty_evict_bytes
              << std::endl;
