...
ConcurrentArrayCache::post_ec_degraded_read_entry(FarObjectEntry *entry,
        ...
        !view.split_object || view.alive_mask != kAllSegments;
    {
        ...
        for (size_t segment = 0; segment < ec_read_recovery::kEcReadSegmentCount;
            ...
            // Each selected shard is one generated READ, not one per retry.
            // Direct EC reads bypass the ordinary fetch admission counter.
            client->record_immediate_read_generated(read_bytes);
            uint64_t spin = 0;
