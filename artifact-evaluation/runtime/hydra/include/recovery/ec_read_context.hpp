...
struct alignas(256) EcReadContext {
    ...
    // Logical payload size for an EC-split object.  A non-zero value marks
    // this request as split; byte_count remains the per-fragment READ size.
    uint32_t split_payload_bytes = 0;
    // Physical endpoint-survivor mask.  alive_mask remains the exact fetched
    // mask stored in scratch (normally four bits for a degraded round).
...
public:
    ...
    bool acquire(size_t owner_idx, uint64_t target_local_addr,
        ...
        context->split_payload_bytes = split_payload_bytes;
        context->physical_alive_mask = physical_alive_mask;
