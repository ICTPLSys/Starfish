...
public:
    // Observational counters only; leave other benchmarks' reporting enabled
    // by default. OFF skips both the locality probe and the global atomic RMW.
    static bool deref_stats_enabled() {
        static const bool enabled = [] {
            const char* env = std::getenv("FARLIB_HASHMAP_DEREF_STATS");
...
inline bool ConcurrentHashMap<Key, Val, Hash>::get(const Key &key, Val *val,
    ...
    // Debug-only control: synchronize the snapshot with existing PUT locking.
    if (debug_locked) return get_once.template operator()<true>();
