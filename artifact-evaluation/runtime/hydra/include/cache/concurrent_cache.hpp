...
#include "cache/placement/placement_policy.hpp"
#include "cache/accounting/runtime_types.hpp"
...
inline bool hydra_local_ec_batch_poll_enabled() {
    static const bool enabled = [] {
        return ::FarLib::get_config().is_hydra_mode() &&
               env_flag_or_default("FARLIB_HYDRA_LOCAL_POLL", false);
    }();
    return enabled;
}

inline bool hydra_zero_copy_write_enabled() {
    static const bool enabled = env_flag_or_default("FARLIB_HYDRA_ZERO_COPY_WRITE", true);
    return enabled;
}

inline bool hydra_fast_write_prepare_enabled() {
    static const bool enabled = env_flag_or_default("FARLIB_HYDRA_FAST_WRITE_PREPARE", true);
    return enabled;
}

// Bounded per-worker WRITE chains, enabled for Hydra by default. An explicit
// zero remains available for a same-binary diagnostic control.
inline size_t hydra_write_batch_limit() {
    static const size_t limit = []() -> size_t {
        if (!::FarLib::get_config().is_hydra_mode()) return 0;
        const char *value = std::getenv("FARLIB_HYDRA_WRITE_BATCH");
        if (value == nullptr) return hydra::WriteBatch::kCapacity;
        const long parsed = std::strtol(value, nullptr, 0);
        return parsed <= 0 ? 0 : std::min<size_t>(parsed, hydra::WriteBatch::kCapacity);
    }();
    return limit;
}
            ...
            do {
                ...
                if (retry_count > 1024) {
                    if (::FarLib::get_config().is_hydra_mode()) {
                        std::array<size_t, 8> states{};
                        size_t pinned = 0, invalid = 0;
                        {
                            std::lock_guard<std::mutex> lock(hydra_pages_mutex_);
                            for (const auto &page : hydra_pages_) {
                                const auto state = page->owner.load_state();
                                ++states[state.state];
                                pinned += state.ref_cnt != 0;
                                invalid += state.invalid != 0;
                            }
                        }
                        auto b = ::FarLib::allocator::global_heap.get_bin_snapshot(alloc_bin);
                        std::cerr << "hydra.alloc_exhausted bin=" << alloc_bin
                                  << " free_regions=" << b.free_regions
                                  << " full_regions=" << b.full_regions
                                  << " usable=" << b.target_usable_regions
                                  << " can_allocate=" << b.can_allocate_now
                                  << " free_bytes=" << ::FarLib::allocator::global_heap.get_free_size()
                                  << " pinned=" << pinned << " invalid=" << invalid
                                  << " states=";
                        for (auto n : states) std::cerr << n << ',';
                        std::cerr << std::endl;
                        ::FarLib::allocator::global_heap.print_async_full_returns("allocation_exhausted");
...
public:
    ...
    bool mark_recomputable(far_obj_t obj) {
        const auto &config = ::FarLib::get_config();
        if (obj.is_null() || obj.get_entry_ptr()->hydra_packed() || !config.exclusive_cache ||
    ...
    struct EvictBufferSet {
        ...
        size_t server_count;
        // Stable logical producer, not the OS thread on which its fibre runs.
        size_t hydra_owner_id = SIZE_MAX;
        
        EvictBufferSet() : buffers(nullptr), server_count(0) {}
        
        void init(size_t count, size_t owner_id = SIZE_MAX) {
            server_count = count;
            if (::FarLib::get_config().is_hydra_mode() && owner_id == SIZE_MAX)
                ERROR("Hydra Evict buffer requires a logical worker owner");
            hydra_owner_id = owner_id;
            buffers.reset(new EvictBuffer[server_count]);
            // Hydra emits only single-sided EC writes, never Sponge commit
            // RPCs. Keep other modes unchanged; do not allocate/zero these
            // large, unused arrays on every Hydra owned-reclaim batch.
            if (::FarLib::get_config().ft_enabled() &&
                !::FarLib::get_config().is_hydra_mode()) {
        ...
        void flush_all(size_t client_idx, ConcurrentArrayCache* cache) {
            cache->flush_hydra_write_batch(hydra_owner_id);
    ...
    void mark_phase(uint32_t timestamp,
                    ...
                    std::atomic_size_t *legacy_region_budget = nullptr,
                    std::atomic_bool *mark_stop_after_evict = nullptr);
    void evict_post_phase(uint32_t timestamp, size_t hydra_owner_id = SIZE_MAX);
    ...
    void evict_ready_worker_logic(
        ...
        std::atomic_int64_t &posted_wrs_total, size_t hydra_owner_id = SIZE_MAX);
    void evict_drain_phase();
    void drain_eviction_completions_for_shutdown();
    void evacuate_phase(uint32_t timestamp, size_t hydra_owner_id = SIZE_MAX);
    void gc_phase(
        uint32_t timestamp,
    ...
    void deallocate(FarObjectEntry &entry, size_t size) {
        if (entry.hydra_packed()) {
            if constexpr (OldAccessor) entry.unpin();
            hydra_release_handle(entry);
            if constexpr (DeallocateEntry) deallocate_entry(&entry);
            return;
        }
        int invalid_spin = 0;
    retry_load:
...
public:
    ConcurrentArrayCache(void *local_buf, size_t local_buf_size,
        ...
        if (config.is_hydra_mode() &&
            (::FarLib::allocator::six_group::enabled() ||
             ...
             ::FarLib::simple_region_budget::enabled())) {
            ERROR("hydra does not use behavior-group classification; disable Design2/fixed-six environment flags");
        }
        if (config.is_hydra_mode()) {
            hydra::page_codec_initialize();
            std::cerr << "hydra.policy backup=0 behavior_groups=0 resident_budget_bytes="
                      << config.local_resident_budget_bytes
                      << " codec=isa-l rs=4+2 encode_tables=process_once\n";
        }
        ...
        // each call site.
        const bool ec_batch_config = config.is_ec_batch_mode();
    ...
    void quiesce_background_evacuation() {
        ...
        // Producers have joined. Only now may the shutdown thread inspect or
        // flush another worker's private queue (normally already empty).
        if (hydra_write_batch_limit() != 0) {
            for (size_t owner = 0; owner < hydra_write_ring_.owner_count(); ++owner)
    ...
    static FarObjectEntry &get_entry_of(far_obj_t obj) {
        return *obj.get_entry_ptr()->hydra_owner();
    }
    static far_obj_t runtime_object(far_obj_t obj) {
        auto *handle = obj.get_entry_ptr();
        if (handle != nullptr && handle->hydra_packed()) {
            return {.size = hydra::kPageBytes,
                    .obj_id = reinterpret_cast<uint64_t>(handle->hydra_owner())};
        }
        return obj;
    }
    static size_t object_offset(far_obj_t obj) {
        return obj.get_entry_ptr()->hydra_offset();
    }
    static void *object_local_addr(far_obj_t obj) {
        return obj.get_entry_ptr()->local_addr();
    }
    static ::FarLib::allocator::BlockHead *object_block(
        const FarObjectEntry *entry, const void *object_addr) {
        const void *base = entry->hydra_packed()
            ? entry->hydra_owner()->local_addr() : object_addr;
        return static_cast<::FarLib::allocator::BlockHead *>(const_cast<void *>(base)) - 1;
    }
    ...
    std::pair<far_obj_t, void *> allocate(FarObjectEntry *entry, size_t size,
        ...
        return allocate_runtime_object<Lite>(entry, size, dirty, scope, logical_owner_id);
    }

    template <bool Lite = false>
    std::pair<far_obj_t, void *> allocate_runtime_object(
        FarObjectEntry *entry, size_t size, bool dirty, DereferenceScope &scope,
        uint32_t logical_owner_id = 0) {
    ...
    void deallocate(far_obj_t obj) {
        deallocate<true, OldAccessor>(*obj.get_entry_ptr(), obj.size);
    }
...
public:
    ...
    bool post_ec_batch_group(const ec_batch::EcGroupSendRecord &record,
                             ...
                             size_t hydra_owner_id = SIZE_MAX,
                             bool page_prevalidated = false,
                             uint64_t prepared_page_token = 0);
    void enqueue_hydra_write_group(const ec_batch::EcGroupSendRecord &record,
                                   uint64_t token, size_t owner,
                                   size_t client_idx, size_t qp_idx);
    void flush_hydra_write_batch(size_t owner);
    size_t hydra_write_batch_pending(size_t owner) const;
    ...
    static bool ec_batch_uses_split(size_t size) {
        ...
        return size > ec_batch::kEcBatchStagingMaxSlotSize ||
               !::FarLib::get_config().ft_small_object(size);
    }
    ...
    bool reuse_ec_recovery_endpoint_is_dead(uint64_t remote_addr,
        ...
        if (object_size != 0 && ec_batch_uses_split(object_size) &&
            remote_allocator.small_object_stripe_manager().owns(remote_addr)) {
            ...
            if (!manager.slot_group_is_split(remote_addr) ||
                ...
                !manager.get_slot_group_layout(
                    {layout.stripe_id, layout.slot_id}, &group)) {
                // An old flat/invalid large-object copy cannot satisfy the
                // split protection contract. Re-encode on its next eviction.
                return true;
            }
    ...
    size_t ec_split_buffer_in_use() const {
        return ec_split_buffers_.in_use() + hydra_write_ring_.in_use();
    }
...
private:
    ...
    // Owner-local multi-buffer write contexts. The old table remains for
    // inherited non-page EC paths; Hydra pages never acquire its global quota.
    hydra::WriteRing hydra_write_ring_{
        std::max<size_t>(1, ::FarLib::get_config().evacuate_thread_cnt),
        ::FarLib::get_config().is_hydra_mode()
            ? hydra::WriteRing::kDoubleBufferedSlots
            : hydra::WriteRing::kLegacySlotsPerOwner,
        ::FarLib::get_config().is_hydra_mode()};
    // Queue metadata belongs to one logical worker, never a CQ poller or OS
    // thread. The ring owns payload lifetime; these queues hold pointer values.
    std::unique_ptr<hydra::WriteBatch[]> hydra_write_batches_{
        std::make_unique<hydra::WriteBatch[]>(hydra_write_ring_.owner_count())};
    size_t ec_write_tokens_in_use() const {
        return ec_batch_tokens_.in_use() + hydra_write_ring_.in_use();
    }
    ...
    // Telemetry only: avoid a shared cache line on every page/CQE. Ownership
    // and drain decisions still use the ring/table, never these aggregates.
    hydra::ShardedDiagnosticCounter ec_batch_groups_posted_{0};
    hydra::ShardedDiagnosticCounter hydra_zero_copy_write_groups_{0};
    hydra::ShardedDiagnosticCounter hydra_write_data_copy_bytes_{0};
    hydra::ShardedDiagnosticCounter hydra_write_prepare_checks_{0};
    hydra::ShardedDiagnosticCounter hydra_write_batch_flushes_{0};
    hydra::ShardedDiagnosticCounter hydra_write_batch_post_calls_{0};
    hydra::ShardedDiagnosticCounter hydra_write_batch_accepted_wrs_{0};
    hydra::ShardedDiagnosticCounter hydra_write_batch_partial_posts_{0};
    hydra::ShardedDiagnosticCounter hydra_write_batch_failed_unposted_{0};
    hydra::ShardedDiagnosticCounter ec_batch_groups_completed_{0};
    hydra::ShardedDiagnosticCounter ec_batch_post_rejects_{0};
    ...
    // Hydra-only split-stage polling diagnostics.  The counters are updated
    // only when FARLIB_HYDRA_LOCAL_POLL=1, so the default path has no extra
    // atomic increments.  The local pass always covers every QP/endpoint of
    // the posting client; the fallback is deliberately periodic and does not
    // depend on whether the local pass happened to reap a completion.
    hydra::ShardedDiagnosticCounter ec_batch_hydra_local_poll_passes_{0};
    hydra::ShardedDiagnosticCounter ec_batch_hydra_local_poll_global_fallbacks_{0};
    hydra::ShardedDiagnosticCounter ec_batch_hydra_local_poll_completions_{0};
    ...
    hydra::ShardedDiagnosticCounter ec_candidate_checked_{0};
    hydra::ShardedDiagnosticCounter ec_candidate_true_{0};
    hydra::ShardedDiagnosticCounter ec_diag_stage_called_{0};
    hydra::ShardedDiagnosticCounter ec_diag_stage_ok_{0};
    ...
    hydra::ShardedDiagnosticCounter ec_diag_complete_cqes_{0};
    hydra::ShardedDiagnosticCounter ec_diag_complete_not_last_{0};
    hydra::ShardedDiagnosticCounter ec_diag_complete_last_{0};
    ...
    // Separate writer owners from foreground READ scratch owners.
...
#include "cache/diagnostics/diagnostics.ipp"
#include "cache/placement/fetch_placement.ipp"
#include "cache/placement/placement_planner.ipp"
#include "cache/integration/cache_hooks.ipp"
