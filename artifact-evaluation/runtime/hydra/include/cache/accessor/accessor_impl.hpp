...
inline void UniqueFarPtrBase<T, Impl>::move(FarObjectEntry *other) {
    if (other->hydra_packed()) {
        entry.hydra_take_handle(*other);
        return;
    }
...
inline void UniqueFarPtrBase<T, Impl>::atomic_move_to_and_set_busy(
    ...
    if (entry.hydra_packed()) {
        to.entry.hydra_take_handle(entry);
        EntryStateBits busy{};
        busy.state = BUSY;
        entry.set_state(busy);
        return;
    }
    auto *cache = Cache::get_default();
...
inline LiteAccessor<T, true> UniqueFarPtr<T>::allocate_lite(
    ...
    accessor.block = Cache::object_block(&entry, p);
    accessor.remember_object(this->obj());
...
inline LiteAccessor<T, Mut>::LiteAccessor(far_obj_t obj, DereferenceScope &scope) {
    ...
    block = Cache::object_block(obj.get_entry_ptr(), ptr);
    this->remember_object(obj);
...
inline LiteAccessor<T, Mut>::LiteAccessor(const UniqueFarPtrBase<T, Impl> &uptr,
    ...
    block = Cache::object_block(&uptr.get_entry(), ptr);
    this->remember_object(uptr.obj());
    local_ptr = static_cast<T *>(ptr);
    check(block, local_ptr);
}
...
inline bool LiteAccessor<T, Mut>::async_fetch_slow_path(far_obj_t obj,
    ...
    this->block = Cache::object_block(obj.get_entry_ptr(), local_addr);
    this->remember_object(obj);
    check(block, local_ptr);
    return at_local;
}
...
inline bool LiteAccessor<T, Mut>::async_fetch(const UniqueFarPtr<T> &uptr,
    ...
    if (fast) [[likely]] {
        ...
        this->local_ptr = static_cast<Pointer>(local_addr);
        this->block = Cache::object_block(&uptr.get_entry(), local_addr);
        this->remember_object(uptr.obj());
        check(block, local_ptr);
        return true;
    }
...
inline void LiteAccessor<T, Mut>::sync() {
    ...
    const auto offset = reinterpret_cast<const char *>(local_ptr) -
                        static_cast<const char *>(block->get_object_ptr());
    void *base = Cache::get_entry_of(get_obj()).local_addr();
    this->block = static_cast<allocator::BlockHead *>(base) - 1;
    this->local_ptr = reinterpret_cast<Pointer>(static_cast<char *>(base) + offset);
    check(block, local_ptr);
}
