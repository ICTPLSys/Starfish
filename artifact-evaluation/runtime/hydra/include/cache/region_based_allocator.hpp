...
namespace list_only_six = six_group::list_only_six;
...
struct BlockHead {
    ...
    // A normal RDMA READ carries a generation-tagged user pointer as wr_id. Keep the
    // block resident until that completion (including a flushed/error WC) is
    // consumed; otherwise a late WC can dereference a reclaimed/reused block.
    std::atomic<uint32_t> pending_rdma_reads{0};
    // Fourteen-bit generation embedded in an ordinary READ wr_id.  The
    // generation distinguishes a delayed completion for an old occupant from
    // a new READ that reuses the same allocator address (ABA protection).
    std::atomic<uint16_t> rdma_read_generation{0};
    // Serializes the transition between an empty READ epoch and its first
    // request.  Multiple fibres can otherwise observe pending==0 and publish
    // different generations, causing one legitimate completion to be treated
    // as stale (and leaking its pin).
    std::atomic_flag rdma_read_pin_lock = ATOMIC_FLAG_INIT;
...
static_assert(sizeof(BlockHead) == 24,
              "recovery ownership must not enlarge the object header");

// Ordinary READ wr_id namespace.  EC uses bit 63 (and bit 62 for its read
// namespace); using bit 62 with bit 63 clear keeps the namespaces disjoint
// while leaving the low 48 bits for the canonical user pointer.  Bits 48..61
// carry a non-zero per-block generation, preventing an old CQE from claiming
// a later request after the block address is reused.
inline constexpr uint64_t kNormalReadWrIdTagBit = 1ull << 62;
inline constexpr uint64_t kNormalReadWrIdPointerMask = (1ull << 48) - 1;
inline constexpr unsigned kNormalReadWrIdGenerationShift = 48;
inline constexpr uint16_t kNormalReadWrIdGenerationMask = (1u << 14) - 1;

inline uint16_t next_normal_read_generation(BlockHead *block) {
    uint16_t old = block->rdma_read_generation.load(std::memory_order_relaxed);
    while (true) {
        uint16_t next = static_cast<uint16_t>(
            (static_cast<unsigned>(old) + 1u) &
            kNormalReadWrIdGenerationMask);
        if (next == 0) next = 1;
        if (block->rdma_read_generation.compare_exchange_weak(
                old, next, std::memory_order_acq_rel,
...
// Acquire one ordinary-READ lifetime pin and return the epoch token to put in
// its wr_id.  All requests that overlap the same FETCHING epoch share one
// generation; this is intentional.  Each request still contributes one pin,
...
inline uint16_t acquire_normal_read_pin(BlockHead *block) {
    ...
    uint16_t generation = block->rdma_read_generation.load(
        std::memory_order_relaxed);
    if (pending == 0) {
        generation = next_normal_read_generation(block);
    }
...
// Close the ordinary-READ gate before a degraded read is posted.  The same
// pin lock serializes this transition with a racing pin acquisition: a pin
// that wins first is included in the drain, and a pin that arrives later is
// rejected.
inline void begin_normal_read_recovery(BlockHead *block) {
    while (block->rdma_read_pin_lock.test_and_set(std::memory_order_acquire)) {
    }
    block->normal_read_recovery_active.fetch_or(1, std::memory_order_release);
    block->rdma_read_pin_lock.clear(std::memory_order_release);
}
...
// Replaces the global target->token lookup. The owner bit is also a lifetime
// pin observed by the allocator, separate from ordinary RDMA READ pins.
inline bool try_claim_degraded_read(BlockHead *block) {
    while (block->rdma_read_pin_lock.test_and_set(std::memory_order_acquire)) {}
    uint8_t expected = 1;
    const bool claimed =
        block->pending_rdma_reads.load(std::memory_order_acquire) == 0 &&
        block->normal_read_recovery_active.compare_exchange_strong(
            expected, uint8_t{3}, std::memory_order_acq_rel);
    block->rdma_read_pin_lock.clear(std::memory_order_release);
    return claimed;
}

inline uint64_t encode_normal_read_wr_id(void *local_addr,
                                         uint16_t generation) {
    const uint64_t pointer = reinterpret_cast<uintptr_t>(local_addr);
    ASSERT((pointer & ~kNormalReadWrIdPointerMask) == 0);
    ASSERT(generation != 0 &&
           generation <= kNormalReadWrIdGenerationMask);
    return kNormalReadWrIdTagBit |
           (static_cast<uint64_t>(generation)
            << kNormalReadWrIdGenerationShift) |
           (pointer & kNormalReadWrIdPointerMask);
}

inline bool is_normal_read_wr_id(uint64_t wr_id) {
    return (wr_id & kNormalReadWrIdTagBit) != 0 &&
           (wr_id & (1ull << 63)) == 0;
}

inline void *normal_read_wr_id_local_addr(uint64_t wr_id) {
    return reinterpret_cast<void *>(wr_id & kNormalReadWrIdPointerMask);
}

inline uint16_t normal_read_wr_id_generation(uint64_t wr_id) {
    return static_cast<uint16_t>(
        (wr_id >> kNormalReadWrIdGenerationShift) &
        kNormalReadWrIdGenerationMask);
}
...
struct RegionListNodeBase {
    ...
    RegionListNodeBase(RegionListNodeBase *n)
        : next_region(n), prev_region(n) {}

    RegionListNodeBase *next_region = nullptr;
    RegionListNodeBase *prev_region = nullptr;
};

struct RegionList;
struct RegionGroupBucket;
...
struct alignas(64) RegionHead : public RegionListNodeBase {
    // Main-list ownership is production state: the fixed-six metadata
    // callback uses it to repair the secondary group index without touching
    // registry metadata while a list lock is held.
    std::atomic<RegionList *> owner_list{nullptr};
    RegionHead *six_group_prev = nullptr;
    RegionHead *six_group_next = nullptr;
    uint32_t six_indexed_group_id = 0;
    ...
    std::atomic<uint32_t> pending_allocation_publications;
    BlockHead *free_list;
    ...
    BlockHead *allocate(cache::far_obj_t obj, bool pin_publication = false);
...
struct RegionGroupBucket {
    RegionHead *first = nullptr;
    RegionHead *last = nullptr;
};

struct SimpleBudgetExtractStats {
    size_t scanned = 0;
    size_t eligible = 0;
    size_t busy = 0;
};
...
struct RegionList {
    ...
    // Cursor for bounded native-thread scans of public usable lists.  It is
    // protected by lock and advanced on every unlink so an ineligible prefix
    // cannot starve later donor Regions.
    RegionListNodeBase *simple_budget_scan_cursor;

    // Optional O(1) group discovery for usable lists.  The map is protected
    // by lock; RegionHead stores only intrusive links and the indexed key, so
    // map rehash never invalidates a live node's membership.
    bool six_group_index_enabled = false;
    std::unordered_map<uint32_t, RegionGroupBucket> six_group_buckets;

    // List-only six-way ablation.  The parent is a routing wrapper with no
    // intrusive nodes of its own; each child owns an independent list and
    // cursor.  Production placement is intentionally fixed to child zero.
    // Children 1..5 are retained so GC/evict traversal can aggregate every
    // physical list, including test-seeded nonzero children.
    std::unique_ptr<RegionList[]> list_only_six_children;
    RegionList *list_only_six_parent = nullptr;
    // Parent-only cursors for the exploratory six-independent-list mode.
    // Children never consult these atomics.
    std::atomic<uint32_t> list_only_six_next_push{0};
    std::atomic<uint32_t> list_only_six_next_pop{0};
    ...
    void enable_six_group_index();
    void enable_list_only_six_children();
    RegionList *list_only_six_child(size_t slot) const;
    size_t reclassify_simple_heat();
    ...
    RegionHead *pop_for_six_group(uint32_t group_id, EligibleFn &&eligible);
    template <typename EligibleFn>
    RegionHead *pop_for_group(uint32_t group_id, EligibleFn &&eligible);
    void add_six_group_index_locked(RegionHead *node, bool front = false);
    void remove_six_group_index_locked(RegionHead *node);
    void reindex_six_group_locked(RegionHead *node);
    void set_owner_locked(RegionHead *node, bool front = false);
    void clear_owner_locked(RegionHead *node);
    template <typename EligibleFn>
    ...
    size_t size() const {
        if (list_only_six_children) {
            ...
            for (size_t i = 0; i < list_only_six::kChildCount; ++i)
                result += list_only_six_children[i].size();
            return result;
        }
    ...
    // Add publicly reusable free bytes to the two simple-budget supply
    // classes.  The list lock and placement lock are held while free_size()
    // is read; callers must not use this for private ThreadHeap slots.
    void append_simple_budget_supply(simple_region_budget::Supply &supply);
    size_t extract_simple_budget_usable(
...
private:
    ...
    void simple_budget_cursor_on_unlink_locked(RegionListNodeBase *node,
                                               RegionListNodeBase *next);
};
...
public:
    ...
    // Called once per completed simple-heat window.  The implementation
    // snapshots only globally published Region-list supply before forwarding
    // it to the opt-in local controller.
...
// Keep the allocation-to-entry-publication interval visible to the marker.
// This owns no allocator lock, and can safely follow a migrating fibre.
class PendingLocalAllocation {
    BlockHead *block_;
    bool pinned_;

public:
    PendingLocalAllocation(BlockHead *block, bool pinned)
        : block_(block), pinned_(pinned) {}
    PendingLocalAllocation(const PendingLocalAllocation &) = delete;
    PendingLocalAllocation &operator=(const PendingLocalAllocation &) = delete;
    PendingLocalAllocation(PendingLocalAllocation &&other) noexcept
        : block_(other.block_), pinned_(other.pinned_) {
        other.pinned_ = false;
    }
    ~PendingLocalAllocation() {
        if (!pinned_) return;
        const auto previous =
            block_to_region(block_)->pending_allocation_publications.fetch_sub(
                1, std::memory_order_release);
        ASSERT(previous != 0);
    }
    void *get() const { return static_cast<void *>(block_ + 1); }
};
