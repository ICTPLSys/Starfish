    ...
    case EntryState::LOCAL:
        ...
        if (::FarLib::get_config().exclusive_cache) {
            ...
            while (true) {
                ...
                // Our write ref was already dropped. A new writeback/pin may
                // now own this page; do not erase its newly published anchor.
                // CAS below includes ref_cnt, closing a race after this check.
                if (entry.hydra_page() &&
                    !hydra::page_write_cleanup_may_lock(cleanup_state)) return;
