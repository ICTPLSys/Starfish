...
// Hydra's six-WR page eviction makes a large owned batch a long supply gap.
// Keep controls separate so batch size and early notification can be tested
// independently. Non-Hydra runtimes retain the inherited 64-region policy.
inline size_t hydra_owned_reclaim_batch_limit() {
    if (!::FarLib::get_config().is_hydra_mode()) return 64;
    static const size_t limit = [] {
        const char *value = std::getenv("FARLIB_HYDRA_RECLAIM_BATCH");
        if (value == nullptr) return size_t{64};
        char *end = nullptr;
        const auto parsed = std::strtoul(value, &end, 10);
        if (end == value || *end != '\0' || parsed < 1 || parsed > 64)
            ERROR("hydra: FARLIB_HYDRA_RECLAIM_BATCH must be in [1,64]");
        return static_cast<size_t>(parsed);
    }();
    return limit;
}

inline bool hydra_reclaim_notify_enabled() {
    static const bool enabled = [] {
        const char *value = std::getenv("FARLIB_HYDRA_RECLAIM_NOTIFY");
        return value != nullptr && std::strcmp(value, "1") == 0;
    }();
    return enabled && ::FarLib::get_config().is_hydra_mode();
}
                 ...
                 &next_task_idx, &worker_deferred](size_t worker_id) {
                    cache->evict_ready_worker_logic(
                        *ready_tasks, next_task_idx, current_safe_epoch,
                        worker_deferred[worker_id], *posted_wrs_total, worker_id);
            };
...
inline void ConcurrentArrayCache::evacuate_work() {
        ...
        while (true) {
            ...
            std::function<void(size_t)> fn_evacuate = [this, &timestamp](size_t worker_id) {
                evacuate_phase(timestamp, worker_id);
            };
...
inline void ConcurrentArrayCache::evict_post_phase(uint32_t timestamp, size_t hydra_owner_id) {
        ...
        buffer_set.init(::FarLib::get_config().server_count, hydra_owner_id);
...
inline void ConcurrentArrayCache::evict_post_worker_logic(
    ...
    std::atomic_bool *evict_stop_after_mark, size_t hydra_owner_id) {
        if (ignore_safe_epoch && exclusive_owned_batch_reclaim_enabled()) {
            std::vector<::FarLib::allocator::EvictTask> collected_tasks;
            const size_t batch_limit = hydra_owned_reclaim_batch_limit();
            while (true) {
                ...
                ::FarLib::allocator::global_heap.collect_evict_tasks(
                    ...
                    legacy_scan_cursors_enabled(), eligibility,
                    nullptr,
        ...
        while (true) {
            ...
            size_t pending = buffer_set.pending_count() +
                             hydra_write_batch_pending(hydra_owner_id);
            if (pending > 0) {
                auto client_idx = rdma::thread_info.thread_id;
                auto flush_start = get_cycles();
                buffer_set.flush_all(client_idx, this);
...
inline void ConcurrentArrayCache::evict_ready_worker_logic(
    ...
    std::vector<::FarLib::allocator::EvictTask> &deferred_tasks,
    std::atomic_int64_t &posted_wrs_total, size_t hydra_owner_id) {
        ...
        for (auto &task : processed_tasks) {
            ...
            if (region->marked_list == nullptr) {
                ...
                // and repurpose the region immediately after the push.
                published_supply |= region->is_empty() || region->can_allocate();
        ...
        profile::count_gc();
        // Regions are already published, all list/CQ locks are released, and
        // the normal allocation predicate still decides whether a waiter can
        // proceed. Do not make foreground fibres wait for unrelated workers
        // to finish a whole collector round after usable supply exists.
        if (published_supply && hydra_reclaim_notify_enabled() &&
            mutator_waiters_need_progress()) {
            notify_mutators_ready();
        }
    }
...
inline void ConcurrentArrayCache::evacuate_phase(uint32_t timestamp, size_t hydra_owner_id) {
        int64_t evict_start = profile::start_evict();
        evict_post_phase(timestamp, hydra_owner_id);
        evict_drain_phase();
...
inline void ConcurrentArrayCache::on_demand_invoke_eviction(size_t alloc_bin) {
        ...
        // Hydra concentrates allocations in the page bin. A notification is
        // not evidence that this bin has usable supply; use the predicate loop
        // below instead of exhausting allocation retries on early wakeups.
        if (!::FarLib::get_config().is_hydra_mode() &&
            optimized_legacy_exclusive_pipeline_enabled() &&
            (::FarLib::get_config().exclusive_cache ||
