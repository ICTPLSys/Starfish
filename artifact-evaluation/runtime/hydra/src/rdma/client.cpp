...
void ClientControl::release_sponge_rpc_resources() {
    sponge_commit_send_rings.clear();
    sponge_commit_send_heads.clear();
    sponge_commit_send_base = nullptr;
    sponge_commit_send_count = 0;
    sponge_ack_recv_base = nullptr;
    sponge_ack_recv_count = 0;
    if (sponge_rpc_mr != nullptr) {
        CHECK_ERR(ibv_dereg_mr(sponge_rpc_mr));
        sponge_rpc_mr = nullptr;
    }
    if (sponge_rpc_mem != nullptr) {
        std::free(sponge_rpc_mem);
        sponge_rpc_mem = nullptr;
    }
}

void ClientControl::init_sponge_rpc_resources() {
    if (!config.ft_enabled() || server_count == 0) return;
    const size_t commit_per_endpoint = kSpongeCommitSendDepthPerEndpoint;
    const size_t ack_per_endpoint = kSpongeAckRecvDepthPerEndpoint;
    const size_t commit_count = commit_per_endpoint * server_count;
    const size_t ack_count = ack_per_endpoint * server_count;
    const size_t commit_bytes =
        commit_count * sizeof(SpongeClientCommitWireSendSlot);
    const size_t ack_bytes = ack_count * sizeof(SpongeAckWireRecvSlot);
    const size_t total_bytes = commit_bytes + ack_bytes;
    sponge_rpc_mem = std::aligned_alloc(64, total_bytes);
    if (sponge_rpc_mem == nullptr) {
        ERROR("client sponge rpc memory allocation failed");
    }
    std::memset(sponge_rpc_mem, 0, total_bytes);
    sponge_rpc_mr = ibv_reg_mr(pd.protection_domain, sponge_rpc_mem,
                               total_bytes,
                               IBV_ACCESS_LOCAL_WRITE |
                                   IBV_ACCESS_REMOTE_WRITE);
    if (sponge_rpc_mr == nullptr) {
        ERROR("client sponge rpc memory registration failed");
    }
    auto *commit_base =
        static_cast<SpongeClientCommitWireSendSlot *>(sponge_rpc_mem);
    auto *ack_base = reinterpret_cast<SpongeAckWireRecvSlot *>(
        static_cast<uint8_t *>(sponge_rpc_mem) + commit_bytes);
    sponge_commit_send_base = commit_base;
    sponge_commit_send_count = commit_count;
    sponge_ack_recv_base = ack_base;
    sponge_ack_recv_count = ack_count;

    sponge_commit_send_rings.assign(server_count, {});
    sponge_commit_send_heads.assign(server_count, 0);
    for (size_t ep = 0; ep < server_count; ep++) {
        auto &ring = sponge_commit_send_rings[ep];
        ring.reserve(commit_per_endpoint);
        for (size_t i = 0; i < commit_per_endpoint; i++) {
            auto &slot = commit_base[ep * commit_per_endpoint + i];
            new (&slot) SpongeClientCommitWireSendSlot();
            slot.reset();
            slot.endpoint_idx = static_cast<uint16_t>(ep);
            ring.push_back(&slot);
        }
    }

    // Every endpoint's control QP gets its own ACK receive slots.
    for (size_t ep = 0; ep < server_count; ep++) {
        for (size_t i = 0; i < ack_per_endpoint; i++) {
            auto &slot = ack_base[ep * ack_per_endpoint + i];
            new (&slot) SpongeAckWireRecvSlot();
            slot.reset();
            slot.endpoint_idx = static_cast<uint16_t>(ep);
            if (!post_sponge_ack_recv_slot_internal(slot)) {
                ERROR("client sponge ack recv slot post failed");
            }
        }
    }
    INFO(("rdma.sponge_rpc.init commit_slots=" + std::to_string(commit_count) +
          " ack_recv_slots=" + std::to_string(ack_count) +
          " bytes=" + std::to_string(total_bytes)).c_str());
}

bool ClientControl::post_sponge_ack_recv_slot_internal(
    SpongeAckWireRecvSlot &slot) {
    QueuePair *qp = get_endpoint_control_qp(slot.endpoint_idx);
    if (qp == nullptr || sponge_rpc_mr == nullptr) return false;
    ibv_sge sge{};
    sge.addr = reinterpret_cast<uint64_t>(slot.bytes.data());
    sge.length = static_cast<uint32_t>(slot.bytes.size());
    sge.lkey = sponge_rpc_mr->lkey;
    ibv_recv_wr recv_wr{};
    recv_wr.wr_id = reinterpret_cast<uint64_t>(&slot);
    recv_wr.next = nullptr;
    recv_wr.sg_list = &sge;
    recv_wr.num_sge = 1;
    ibv_recv_wr *bad_wr = nullptr;
    const int ret = ibv_post_recv(qp->queue_pair, &recv_wr, &bad_wr);
    return ret == 0;
}

bool ClientControl::owns_sponge_commit_send_slot(uint64_t wr_id) const {
    if (sponge_commit_send_base == nullptr || sponge_commit_send_count == 0) {
        return false;
    }
    const uintptr_t base =
        reinterpret_cast<uintptr_t>(sponge_commit_send_base);
    const uintptr_t ptr = static_cast<uintptr_t>(wr_id);
    const size_t bytes =
        sponge_commit_send_count * sizeof(SpongeClientCommitWireSendSlot);
    if (ptr < base || (ptr - base) >= bytes) return false;
    return ((ptr - base) % sizeof(SpongeClientCommitWireSendSlot)) == 0;
}

bool ClientControl::owns_sponge_ack_recv_slot(uint64_t wr_id) const {
    if (sponge_ack_recv_base == nullptr || sponge_ack_recv_count == 0) {
        return false;
    }
    const uintptr_t base = reinterpret_cast<uintptr_t>(sponge_ack_recv_base);
    const uintptr_t ptr = static_cast<uintptr_t>(wr_id);
    const size_t bytes = sponge_ack_recv_count * sizeof(SpongeAckWireRecvSlot);
    if (ptr < base || (ptr - base) >= bytes) return false;
    return ((ptr - base) % sizeof(SpongeAckWireRecvSlot)) == 0;
}

SpongeClientCommitWireSendSlot *ClientControl::acquire_sponge_commit_send_slot(
    size_t endpoint_idx) {
    if (endpoint_idx >= sponge_commit_send_rings.size()) return nullptr;
    std::lock_guard<std::mutex> lock(sponge_send_mutex);
    auto &ring = sponge_commit_send_rings[endpoint_idx];
    if (ring.empty()) return nullptr;
    size_t &head = sponge_commit_send_heads[endpoint_idx];
    for (size_t attempt = 0; attempt < ring.size(); attempt++) {
        const size_t idx = (head + attempt) % ring.size();
        SpongeClientCommitWireSendSlot *slot = ring[idx];
        if (slot->in_use.load(std::memory_order_acquire)) continue;
        slot->reset();
        slot->endpoint_idx = static_cast<uint16_t>(endpoint_idx);
        slot->in_use.store(true, std::memory_order_release);
        head = (idx + 1) % ring.size();
        return slot;
    }
    return nullptr;
}

bool ClientControl::post_sponge_commit_batch(
    size_t endpoint_idx, size_t qp_idx, const SpongeCommitBatchBuffer &batch) {
    if (sponge_rpc_mr == nullptr) return false;
    if (batch.header.record_count == 0) return false;
    SpongeClientCommitWireSendSlot *slot =
        acquire_sponge_commit_send_slot(endpoint_idx);
    if (slot == nullptr) return false;
    QueuePair *qp = get_endpoint_control_qp(endpoint_idx);
    if (qp == nullptr) {
        slot->reset();
        return false;
    }
    const size_t wire_bytes = batch.wire_bytes();
    if (wire_bytes == 0 || wire_bytes > slot->bytes.size() ||
        !batch.pack_wire(slot->bytes.data(), slot->bytes.size())) {
        slot->reset();
        return false;
    }
    slot->byte_len = static_cast<uint32_t>(wire_bytes);
    slot->qp_idx = static_cast<uint16_t>(qp_idx);
    ibv_sge sge{};
    sge.addr = reinterpret_cast<uint64_t>(slot->bytes.data());
    sge.length = static_cast<uint32_t>(wire_bytes);
    sge.lkey = sponge_rpc_mr->lkey;
    ibv_send_wr send_wr{};
    send_wr.wr_id = reinterpret_cast<uint64_t>(slot);
    ...
    const int ret = ibv_post_send(qp->queue_pair, &send_wr, &bad_wr);
    if (ret != 0) {
        slot->reset();
        return false;
    }
...
bool ClientControl::release_sponge_commit_send_slot(uint64_t wr_id) {
    auto *control = get_default();
    if (control == nullptr) return false;
    if (!control->owns_sponge_commit_send_slot(wr_id)) return false;
    auto *slot = reinterpret_cast<SpongeClientCommitWireSendSlot *>(wr_id);
    if (slot->magic != kSpongeClientCommitWireSendSlotMagic) return false;
    slot->reset();
    return true;
}

bool ClientControl::decode_sponge_ack_recv_slot(uint64_t wr_id,
                                                uint32_t byte_len,
                                                SpongeAckBatchView &view,
                                                SpongeAckWireRecvSlot **slot_out) {
    view = SpongeAckBatchView{};
    if (slot_out != nullptr) *slot_out = nullptr;
    auto *control = get_default();
    if (control == nullptr) return false;
    if (!control->owns_sponge_ack_recv_slot(wr_id)) return false;
    auto *slot = reinterpret_cast<SpongeAckWireRecvSlot *>(wr_id);
    if (slot->magic != kSpongeAckWireRecvSlotMagic) return false;
    if (slot_out != nullptr) *slot_out = slot;
    if (byte_len == 0 || byte_len > slot->bytes.size()) return false;
    return view.parse(slot->bytes.data(), byte_len);
}

bool ClientControl::repost_sponge_ack_recv_slot(uint64_t wr_id) {
    if (!owns_sponge_ack_recv_slot(wr_id)) return false;
    auto *slot = reinterpret_cast<SpongeAckWireRecvSlot *>(wr_id);
    if (slot->magic != kSpongeAckWireRecvSlotMagic) return false;
    const uint16_t endpoint_idx = slot->endpoint_idx;
    slot->reset();
    slot->endpoint_idx = endpoint_idx;
    return post_sponge_ack_recv_slot_internal(*slot);
}
