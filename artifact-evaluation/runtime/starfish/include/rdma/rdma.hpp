...
struct __attribute__((packed)) ServerPeerConnectionInfo {
    uint32_t psn;
    uint16_t lid;
...
static_assert(offsetof(ServerPeerConnectionInfo, qpn) ==
              sizeof(ServerPeerConnectionInfo));
...
struct QueuePair {
    ...
    QueuePair(Context &ctx, CompleteQueue &send_cq, CompleteQueue &recv_cq,
              ProtectionDomain &pd, const Configure &config) {
        init(ctx, send_cq, recv_cq, pd, config);
    }
    ...
    void init(Context &ctx, CompleteQueue &cq, ProtectionDomain &pd,
              const Configure &config) {
        init(ctx, cq, cq, pd, config);
    }

    void init(Context &ctx, CompleteQueue &send_cq, CompleteQueue &recv_cq,
              ProtectionDomain &pd, const Configure &config) {
        ibv_qp_cap qp_cap = {
