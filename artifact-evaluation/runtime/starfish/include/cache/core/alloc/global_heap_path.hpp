    ...
    if (list_only_six::enabled()) {
        for (auto &placement_lists : usable_region_list) {
            for (auto &list : placement_lists)
                list.enable_list_only_six_children();
        }
        std::cerr << "allocator.list_only_six enabled=1 fixed_class=" << (simple_region_heat::grouping_enabled() ? -1 : 0)
                  << " group=0 quota=0" << std::endl;
    }
    if (six_group::enabled() && !list_only_six::enabled()) {
        for (auto &placement_lists : usable_region_list) {
            for (auto &list : placement_lists) list.enable_six_group_index();
        }
        for (auto &list : full_region_list) list.enable_six_group_index();
        free_region_list.enable_six_group_index();
    }
    unallocated_offset = 0;
    heap_size = size;
    heap = buffer;
...
inline size_t GlobalHeap::reclassify_simple_heat() {
    // Native monitor thread must not enter libfibre's yield operation.
    ...
    if (list_only_six::grouped()) {
        for (size_t placement = 0; placement < RegionPlacementCount;
            ...
            for (size_t bin = 0; bin < RegionBinCount; ++bin) {
                auto &list = usable_region_list[placement][bin];
                const size_t total = list.size();
                if (total == 0) continue;
                std::cout << "allocator.list_only_six_window placement="
                          << placement << " bin=" << bin
                          << " total=" << total;
                for (size_t child = 0; child < list_only_six::kChildCount;
                     ++child) {
                    const auto *child_list = list.list_only_six_child(child);
                    std::cout << " child" << child << "="
                              << (child_list == nullptr ? 0
                                                         : child_list->size());
                }
                std::cout << std::endl;
            }
    ...
    size_t moved=0;
    for(auto &placement_lists:usable_region_list)
        for(auto &list:placement_lists)moved+=list.reclassify_simple_heat();
    return moved;
}

inline void GlobalHeap::tick_simple_budget(uint64_t window) {
    if (!simple_region_budget::enabled()) return;

    // The monitor worker is a native thread; list-lock contention must not
    // attempt libfibre's yield path even when heat relinking is disabled.
    disable_region_list_fibre_yield_for_current_thread();

    // Snapshot only globally published USABLE Regions.  Full, free-list and
    // private ThreadHeap descriptors are not available to an exact requested
    // bin pop, so counting them as donor slack would overstate a transfer's
    // safety.  Each child/list lock is held while its Region free_size is
    // read; the resulting values are an intentionally conservative public
    // slack estimate.
    simple_region_budget::Supply supply{};
    for (size_t placement = 0; placement < RegionPlacementCount;
         ++placement) {
        for (size_t bin = 0; bin < RegionBinCount; ++bin) {
            usable_region_list[placement][bin]
                .append_simple_budget_supply(supply);
        }
    }
    simple_region_budget::local().tick(supply, window);
    if (simple_region_budget::mode() != simple_region_budget::Mode::Adaptive)
        return;

    const auto actuation_start = std::chrono::steady_clock::now();
    simple_region_budget::Supply remaining =
        simple_region_budget::local().pending_transfers();
    constexpr size_t kMaxActuationRegions = 4096;
    constexpr size_t kMaxActuationScans = 4096;
    size_t scanned = 0;
    size_t eligible = 0;
    size_t busy = 0;
    size_t applied = 0;
    ...
    const bool have_pending = [&] {
        for (const auto &bin : remaining)
            for (size_t c = 0; c < simple_region_budget::classes(); ++c)
                if (bin[c] != 0) return true;
        return false;
    }();
    if (have_pending) {
        size_t pending_bins = 0;
        for (const auto &per_bin : remaining)
            pending_bins += std::any_of(
                per_bin.begin(), per_bin.begin() + simple_region_budget::classes(),
                [](uint64_t n) { return n != 0; });
        size_t pending_bins_left = pending_bins;
        size_t scan_budget = kMaxActuationScans;
        size_t candidate_budget = kMaxActuationRegions;
        for (size_t bin = 0; bin < simple_region_budget::kBins &&
                             scan_budget != 0 && candidate_budget != 0;
             ++bin) {
            uint64_t pending_for_bin = 0;
            for (size_t c = 0; c < simple_region_budget::classes(); ++c)
                pending_for_bin += remaining[bin][c];
            if (pending_for_bin == 0) continue;
            const size_t visit_quota = std::max<size_t>(
                1, scan_budget / std::max<size_t>(1, pending_bins_left));
            const size_t candidate_quota = std::min<size_t>(
                candidate_budget,
                std::max<size_t>(
    ...
    for (const auto &bin : pending_after)
        for (size_t c = 0; c < simple_region_budget::classes(); ++c)
            pending_count += bin[c];
...
inline RegionHead *GlobalHeap::allocate_region(
    ...
    RegionHead *region = allocate_region_impl(
...
inline RegionHead *GlobalHeap::allocate_region_impl(
    ...
    auto pop_usable = [&](RegionPlacement placement, const char *debug_tag,
        ...
        if(simple_region_heat::local_routing_enabled()) {
            ...
            if (simple_region_budget::enabled() && class_override < 0) {
                simple_region_budget::local().note_request(
                    bin, requested_class, candidate == nullptr);
            }
        } else if (simple_region_heat::grouping_enabled()) {
            // Unified local routing: the parent wrapper aggregates all six
            // children, so Hot/Cold labels remain visible to reclassification
            // but do not constrain allocation.
            candidate = list.pop();
        } else if (requested_behavior_group_id != 0 && six_group::enabled()) {
            candidate = list.pop_for_six_group(
                requested_behavior_group_id, matches);
            // A behavior-routed miss must not silently consume an arbitrary
            // nonempty Region.  The fixed-six cache layer performs explicit,
            // bounded family fallback and records it; free/new Regions are
            // still available below without retagging a live Region.
            if (candidate == nullptr) return nullptr;
        } else {
            candidate = bind_groups ? list.pop_first_matching(matches)
                                    : list.pop();
        }
    ...
    // Six semantic classes try every active public alternative before
    // committing a fresh Region.  Legacy two-class mode intentionally keeps
    // its v19 exact -> fresh/free -> opposite policy below.
...
retry_alloc:
    ...
    if (simple_region_budget::six_enabled() &&
        ...
        if(auto *fallback=usable_region_list[placement_index(actual_placement)][bin].list_only_six_child(opposite)->pop()) {
            sub_free_size(static_cast<int64_t>(fallback->free_size()));
            return fallback;
        }
    }
    if (reserved_new_resident_region) {
...
inline void GlobalHeap::print_used_memory() {
    ...
    for (size_t placement = 0; placement < RegionPlacementCount;
        ...
        if (list_only_six::grouped()) {
            for (size_t bin = 0; bin < RegionBinCount; ++bin) {
                auto &list = usable_region_list[placement][bin];
                std::cout << "allocator.list_only_six_distribution placement="
                          << placement << " bin=" << bin
                          << " total=" << list.size();
                for (size_t child = 0; child < list_only_six::kChildCount;
                     ++child) {
